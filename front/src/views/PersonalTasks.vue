  ...
  <div class="min-h-full bg-gray-50">
    <div class="w-full py-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-5 mb-8">
        ...
        <div class="stat-card bg-green-50 rounded-lg p-4 flex flex-col items-center justify-center hover:transform hover:-translate-y-1 transition-transform">
          <span class="text-2xl font-bold text-green-600">{{ stats.completed }}</span>
          <span class="text-gray-600 text-sm mt-1">已完成</span>
        </div>
        <div class="stat-card bg-orange-50 rounded-lg p-4 flex flex-col items-center justify-center hover:transform hover:-translate-y-1 transition-transform">
          <span class="text-2xl font-bold text-orange-600">{{ stats.inProgress }}</span>
          <span class="text-gray-600 text-sm mt-1">进行中</span>
        </div>
        <div class="stat-card bg-red-50 rounded-lg p-4 flex flex-col items-center justify-center hover:transform hover:-translate-y-1 transition-transform">
      ...
      <div class="flex gap-5 mb-6" style="min-height: 500px;">
        <!-- 左侧日历 -->
        <div class="flex-1 bg-white rounded-lg border border-gray-200 overflow-hidden modern-calendar">
          <!-- 日历头部 -->
          <div class="flex items-center justify-between px-4 h-14 border-b border-gray-200 bg-gray-50">
            <button @click="previousMonth" class="text-blue-600 text-xl font-bold hover:bg-blue-50 w-8 h-8 rounded-full flex items-center justify-center">&lt;</button>
            <div class="font-bold text-base text-gray-800">{{ currentMonthYear }}</div>
            <button @click="nextMonth" class="text-blue-600 text-xl font-bold hover:bg-blue-50 w-8 h-8 rounded-full flex items-center justify-center">&gt;</button>
          </div>
          
          <!-- 星期标题 -->
          <div class="grid grid-cols-7 bg-white py-3 border-b border-gray-100">
            <div class="flex items-center justify-center text-sm text-red-500 font-medium">日</div>
            <div class="flex items-center justify-center text-sm text-gray-600 font-medium">一</div>
            <div class="flex items-center justify-center text-sm text-gray-600 font-medium">二</div>
            <div class="flex items-center justify-center text-sm text-gray-600 font-medium">三</div>
            <div class="flex items-center justify-center text-sm text-gray-600 font-medium">四</div>
            <div class="flex items-center justify-center text-sm text-gray-600 font-medium">五</div>
            <div class="flex items-center justify-center text-sm text-red-500 font-medium">六</div>
          </div>
        ...
        <!-- 右侧任务区域 -->
        <div class="w-96 bg-white rounded-lg border border-gray-200 flex flex-col">
          <!-- 任务头 -->
          ...
          <div class="flex-1 bg-gray-50 rounded-b-lg p-3 overflow-auto">
            <div v-if="selectedDateTasks.length === 0" class="text-center text-gray-500 py-8">
              该日期暂无任务
            </div>
            ...
            >
              <div class="flex items-start">
                ...
                >
                  <svg v-if="task.status === 'completed'" width="10" height="8" viewBox="0 0 10 8" fill="none">
              ...
              <div class="mt-3">
                ...
                <div class="flex items-center mt-2">
                  <button @click="openNotebookModal(task)" class="text-xs text-blue-600 hover:underline">打开笔记本</button>
      ...
      <div class="mt-4 mb-2">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex space-x-4">
            <button :class="[
              ...
              notesSortBy === 'category' ? 'text-blue-600 border-blue-600 bg-blue-50' : 'text-gray-600 border-gray-300 hover:border-blue-600 hover:text-blue-600'
            ]" @click="notesSortBy = 'category'">按主题分类</button>
            <button :class="[
              'text-sm py-1 px-3 border rounded',
              notesSortBy === 'time' ? 'text-blue-600 border-blue-600 bg-blue-50' : 'text-gray-600 border-gray-300 hover:border-blue-600 hover:text-blue-600'
            ]" @click="notesSortBy = 'time'">按时间排序</button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          >
            ...
            <p class="text-xs text-gray-600 mb-2 line-clamp-2">{{ note.content }}</p>
    ...
    <div v-if="showTaskModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click="closeTaskModal">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md max-h-screen overflow-y-auto" @click.stop>
        ...
        <div class="p-4">
          ...
          <div class="mb-4 border border-blue-600 rounded bg-blue-50 p-3">
            <label class="block text-sm text-blue-600 mb-1 font-medium">自然语言输入</label>
            <div class="flex">
              <input 
              ...
              >
              <button @click="parseNaturalLanguage" class="text-white bg-blue-600 px-3 py-1 rounded text-sm hover:bg-blue-700">
                <iconify-icon icon="mdi:wand" width="16" height="16" class="mr-1"></iconify-icon>
                解析
              </button>
            </div>
          </div>
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-sm text-gray-600 mb-1">任务名称 <span class="text-red-500">*</span></label>
              <input 
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm text-gray-600 mb-1">开始日期 <span class="text-red-500">*</span></label>
                <input 
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm text-gray-600 mb-1">结束日期 <span class="text-red-500">*</span></label>
                <input 
        ...
        <div class="flex justify-end p-4 border-t border-gray-200">
          <button @click="closeTaskModal" class="text-sm text-gray-600 border border-gray-300 py-1.5 px-4 rounded mr-3 hover:bg-gray-50">
            取消
          </button>
          <button @click="saveTask" class="text-sm text-white bg-blue-600 py-1.5 px-4 rounded hover:bg-blue-700">
            保存
          </button>
        </div>
      </div>
    ...
    <!-- 笔记本弹窗 -->
    <div v-if="showNotebookModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click="closeNotebookModal">
      <div :class="[
        ...
        <div class="flex items-center justify-between p-4 border-b border-gray-200">
          <div class="flex items-center">
            <h2 class="text-lg font-bold text-gray-800">{{ currentNotebook.title }}</h2>
            <span :class="[
          ...
          <div class="flex items-center space-x-2">
            <button @click="toggleNotebookFullscreen" class="text-gray-500 hover:text-gray-700">
              <iconify-icon :icon="isNotebookFullscreen ? 'mdi:fullscreen-exit' : 'mdi:fullscreen'" width="20" height="20"></iconify-icon>
            </button>
            <button @click="closeNotebookModal" class="text-gray-500 hover:text-gray-700">
              <iconify-icon icon="mdi:close" width="20" height="20"></iconify-icon>
            </button>
          </div>
        </div>
        <div class="p-4 h-full overflow-hidden flex flex-col">
          <div class="flex items-center justify-between mb-4">
            <div class="text-sm text-gray-500">最后更新: {{ currentNotebook.lastUpdated }}</div>
            <div class="flex space-x-2">
              <button class="text-xs text-blue-600 py-1 px-2 border border-blue-600 rounded hover:bg-blue-50">
              ...
              <button class="text-xs text-blue-600 py-1 px-2 border border-blue-600 rounded hover:bg-blue-50">
                <iconify-icon icon="mdi:format-list-bulleted" width="16" height="16"></iconify-icon>
              </button>
            </div>
          </div>
          ...
          <div class="mt-4 flex items-center justify-between">
            <div v-if="currentNotebook.relatedTask">
              ...
              <span class="text-sm text-blue-600">{{ currentNotebook.relatedTask }}</span>
            </div>
            <div>
              <button class="text-sm text-blue-600 py-1 px-3 border border-blue-600 rounded hover:bg-blue-50">插入图片</button>
              <button @click="saveNotebook" class="ml-2 text-sm text-white bg-blue-600 py-1 px-3 rounded hover:bg-blue-700">保存笔记</button>
            </div>
          </div>
        </div>
      </div>
  ...
  setup() {
    ...
    const notes = ref([
      ...
      },
      {
        id: 3,
        title: '物理实验记录',
        content: '波动光学实验的预习材料和关键步骤记录。需要注意的实验误差来源。',
        date: '2月28日',
        category: '物理',
    ...
    const sortedNotes = computed(() => {
      if (notesSortBy.value === 'time') {
        return [...notes.value].sort((a, b) => new Date(b.lastUpdated) - new Date(a.lastUpdated))
      }
      return [...notes.value].sort((a, b) => a.category.localeCompare(b.category))
    })
